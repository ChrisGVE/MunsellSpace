# Comprehensive Conversion Dataset - Mismatches Analysis V4

## W3 Dataset

### Summary Statistics

| Illuminant | Total | Rust Correct | Rust Unknown | Python Correct | Python Unknown | Python Errors | Rust Accuracy | Python Accuracy |
|------------|-------|--------------|--------------|----------------|----------------|---------------|---------------|-----------------|
| C          |   267 |          214 |            0 |            176 |              2 |             0 |         80.1% |           65.9% |
| D65        |   267 |            0 |            0 |              0 |              0 |             0 |          0.0% |            0.0% |
| F7         |   267 |            0 |            0 |              0 |              0 |             0 |          0.0% |            0.0% |

### Detailed Mismatches

#### Color 1: vivid pink

**Hex:** #FFB5BA

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 0.7R 8.0/6.8 | light pink      | 1.0R 8.0/6.6   | moderate pink     |              |

#### Color 2: very dark red

**Hex:** #3F1728

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 1.0R 1.4/4.2 | very dark purplish red | 1.0R 1.4/4.0   | very dark purplish red |              |

#### Color 3: light grayish red

**Hex:** #AD8884

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 5.7R 5.9/3.4 | pale red        | 6.2R 5.9/3.3   | pale red          |              |

#### Color 4: dark reddish gray

**Hex:** #5C504F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.6R 3.5/0.9 | reddish gray    | N 5.0/         | medium gray       |              |

#### Color 5: vivid yellowish pink

**Hex:** #FFB7A5

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.9R 8.0/6.4 | light yellowish pink | 8.4R 8.0/6.1   | moderate yellowish pink | (+0.0, +4.6) |

#### Color 6: deep yellowish pink

**Hex:** #E66721

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 0.9YR 5.8/13.8 | vivid reddish orange | 1.3YR 5.8/13.4 | vivid reddish orange |              |

#### Color 7: light yellowish pink

**Hex:** #F4C2C2

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 2.1R 8.2/4.6 | light pink      | 2.4R 8.2/4.5   | light pink        |              |

#### Color 8: moderate yellowish pink

**Hex:** #D9A6A9

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 0.9R 7.2/5.0 | moderate pink   | 1.2R 7.2/4.8   | moderate pink     |              |

#### Color 9: light grayish reddish brown

**Hex:** #977F73

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.2YR 5.4/2.3 | pale reddish brown | 3.7YR 5.4/2.2  | pale reddish brown |              |

#### Color 10: vivid orange

**Hex:** #F38400

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 4.0YR 6.5/14.0 | strong orange   | 4.4YR 6.5/13.7 | strong orange     | (0.0, 0.0)   |

#### Color 11: brilliant orange

**Hex:** #FD943F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.6YR 7.0/12.1 | strong orange   | 4.1YR 7.0/11.8 | strong orange     | (+0.5, +0.0) |

#### Color 12: deep brown

**Hex:** #593319

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 5.6YR 2.5/4.7 | moderate brown  | 6.0YR 2.5/4.5  | dark brown        | (+0.0, +0.3) |

#### Color 13: light grayish brown

**Hex:** #958070

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.5YR 5.4/2.2 | pale brown      | 7.1YR 5.4/2.1  | pale brown        |              |

#### Color 14: light brownish gray

**Hex:** #8E8279

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.0YR 5.4/1.2 | brownish gray   | N 5.4/         | medium gray       |              |

#### Color 15: brownish gray

**Hex:** #5B504F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.9R 3.5/0.8 | reddish gray    | N 3.5/         | dark grey         |              |

#### Color 16: vivid orange yellow

**Hex:** #F6A600

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 8.4YR 7.3/12.8 | strong orange yellow | 8.8YR 7.3/12.6 | strong orange yellow | (+0.0, +1.2) |

#### Color 17: deep yellowish brown

**Hex:** #654522

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 8.5YR 3.2/4.6 | dark yellowish brown | 8.9YR 3.2/4.5  | dark yellowish brown |              |

#### Color 18: light grayish yellowish brown

**Hex:** #AE9B82

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 9.9YR 6.4/2.4 | pale yellowish brown | 0.3Y 6.4/2.4   | pale yellowish brown |              |

#### Color 19: dark olive

**Hex:** #403D21

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 8.8Y 2.5/2.8 | grayish olive   | 9.1Y 2.5/2.8   | dark grayish olive | (+0.0, +0.2) |

#### Color 20: light grayish olive

**Hex:** #8C8767

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.7Y 5.5/2.4 | pale olive      | 8.1Y 5.5/2.4   | pale olive        |              |

#### Color 21: light olive gray

**Hex:** #8A8776

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.5Y 5.5/1.3 | olive gray      | N 5.5/         | medium gray       |              |

#### Color 22: deep yellow green

**Hex:** #467129

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.4GY 4.2/6.8 | moderate olive green | 7.4GY 4.2/6.7  | moderate olive green | (+0.0, +0.2) |

#### Color 23: strong olive green

**Hex:** #404F00

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.7GY 3.1/6.3 | moderate olive green | 3.8GY 3.1/6.3  | moderate olive green | (+0.0, +0.7) |

#### Color 24: deep olive green

**Hex:** #232F00

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.8GY 1.7/4.1 | dark olive green | 3.8GY 1.7/4.1  | dark olive green  | (+0.0, +2.9) |

#### Color 25: vivid yellowish green

**Hex:** #27A64C

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 1.1G 5.9/10.8 | strong yellowish green | 0.9G 5.9/10.5  | strong yellowish green | (+0.0, +0.2) |

#### Color 26: very deep yellowish green

**Hex:** #003118

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 1.2G 1.7/4.5 | very dark yellowish green | 1.2G 1.6/4.4   | very dark yellowish green | (+0.0, +2.5) |

#### Color 27: vivid green

**Hex:** #008856

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.4G 4.9/8.4 | strong green    | 3.4G 4.9/8.2   | strong green      |              |

#### Color 28: strong green

**Hex:** #007959

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.1G 4.4/7.0 | moderate green  | 6.1G 4.4/6.7   | moderate green    | (0.0, 0.0)   |

#### Color 29: deep green

**Hex:** #00543D

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 5.9G 3.1/5.7 | dark green      | 5.9G 3.1/5.5   | dark green        | (-0.0, +1.3) |

#### Color 30: light greenish gray

**Hex:** #B2BEB5

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 2.8G 7.5/1.0 | greenish gray   | N 7.5/         | light gray        |              |

#### Color 31: dark greenish gray

**Hex:** #4E5755

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 2.1BG 3.6/0.8 | greenish gray   | N 3.6/         | dark grey         |              |

#### Color 32: vivid bluish green

**Hex:** #008882

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 5.0BG 5.0/6.8 | moderate bluish green | 5.2BG 5.0/6.5  | moderate bluish green | (+0.0, +4.2) |

#### Color 33: strong bluish green

**Hex:** #007A74

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 4.6BG 4.5/6.1 | moderate bluish green | 4.7BG 4.5/5.9  | moderate bluish green | (+0.0, +0.9) |

#### Color 34: deep bluish green

**Hex:** #00443F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.1BG 2.5/4.5 | dark bluish green | 3.2BG 2.5/4.3  | dark bluish green | (+0.0, +2.5) |

#### Color 35: vivid greenish blue

**Hex:** #0085A1

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 4.8B 5.0/7.3 | strong greenish blue | 5.4B 5.0/7.0   | moderate greenish blue | (+0.0, +3.7) |

#### Color 36: strong greenish blue

**Hex:** #007791

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 4.8B 4.5/6.6 | moderate greenish blue | 5.3B 4.5/6.4   | moderate greenish blue | (+0.0, +0.4) |

#### Color 37: deep greenish blue

**Hex:** #2E8495

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 3.0B 5.0/5.9 | moderate greenish blue | 3.5B 5.0/5.7   | moderate greenish blue | (-1.5, +1.1) |

#### Color 38: vivid blue

**Hex:** #00A1C2

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 5.1B 6.0/8.4 | brilliant greenish blue | 5.7B 6.0/8.1   | brilliant greenish blue |              |

#### Color 39: deep blue

**Hex:** #00416A

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 2.1PB 2.6/6.5 | dark blue       | 2.6PB 2.6/6.4  | dark blue         | (+0.0, +0.5) |

#### Color 40: bluish white

**Hex:** #E9E9ED

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 9.3PB 9.2/0.8 | purplish white  | N 5.0/         | medium gray       | (0.0, 0.0)   |

#### Color 41: light bluish gray

**Hex:** #B4BCC0

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 8.6B 7.5/1.0 | bluish gray     | N 7.5/         | light gray        |              |

#### Color 42: dark bluish gray

**Hex:** #51585E

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 0.2PB 3.6/1.0 | bluish gray     | N 3.6/         | dark grey         |              |

#### Color 43: brilliant purplish blue

**Hex:** #6C79B8

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.3PB 5.1/8.9 | light purplish blue | 7.5PB 5.1/8.8  | light purplish blue |              |

#### Color 44: strong purple

**Hex:** #875692

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.4P 4.3/8.9 | moderate purple | 6.3P 4.3/8.8   | moderate purple   | (+0.0, +0.1) |

#### Color 45: light purplish gray

**Hex:** #BFB9BD

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 10.0P 7.5/1.1 | purplish gray   | N 7.5/         | light gray        |              |

#### Color 46: dark purplish gray

**Hex:** #5D555B

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 10.0P 3.6/1.0 | purplish gray   | N 3.6/         | dark grey         |              |

#### Color 47: purplish black

**Hex:** #242124

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 9.4P 1.3/0.4 | black           | N 1.3/         | black             | (+0.0, +0.1) |

#### Color 48: very deep reddish purple

**Hex:** #54194E

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 10.0P 2.0/8.2 | deep reddish purple | 0.4RP 2.0/8.1  | very deep reddish purple | (0.0, 0.0)   |

#### Color 49: brilliant purplish pink

**Hex:** #FFC8D6

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 6.0RP 8.5/5.4 | light purplish pink | 6.1RP 8.5/5.3  | light purplish pink | (+0.0, +3.6) |

#### Color 50: vivid purplish red

**Hex:** #CE4676

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.9RP 4.9/13.4 | vivid purplish pink | 7.9RP 5.0/12.9 | strong purplish pink |              |

#### Color 51: strong purplish red

**Hex:** #B3446C

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.5RP 4.4/11.1 | strong purplish pink | 7.5RP 4.5/10.7 | moderate purplish red |              |

#### Color 52: light grayish purplish red

**Hex:** #AF868E

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | 7.9RP 5.9/4.2 | pale purplish red | 8.0RP 5.9/4.1  | pale purplish red |              |

#### Color 53: dark gray

**Hex:** #555555

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| C          | N 3.6        | dark grey       | N3.6           | (Unknown)         |              |

### Python Errors

## Centore Dataset

### Summary Statistics

| Illuminant | Total | Rust Correct | Rust Unknown | Python Correct | Python Unknown | Python Errors | Rust Accuracy | Python Accuracy |
|------------|-------|--------------|--------------|----------------|----------------|---------------|---------------|-----------------|
| C          |   260 |            0 |            0 |              0 |              0 |             0 |          0.0% |            0.0% |
| D65        |   260 |          242 |            0 |            208 |              0 |             0 |         93.1% |           80.0% |
| F7         |   260 |            0 |            0 |              0 |              0 |             0 |          0.0% |            0.0% |

### Detailed Mismatches

#### Color 1: light grayish red

**Hex:** #B4888D

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 5.2R 6.0/3.6 | pale red        | 5.2R 6.0/3.6   | pale red          |              |

#### Color 2: dark reddish gray

**Hex:** #5D4E53

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 3.9R 3.4/0.9 | reddish gray    | N 3.4/         | dark grey         |              |

#### Color 3: light grayish reddish brown

**Hex:** #9E7F7A

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 3.0YR 5.5/2.4 | pale reddish brown | 3.0YR 5.5/2.4  | pale reddish brown |              |

#### Color 4: light grayish brown

**Hex:** #997F75

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 7.3YR 5.4/2.2 | pale brown      | 7.3YR 5.4/2.2  | pale brown        |              |

#### Color 5: light brownish gray

**Hex:** #928281

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 7.2YR 5.5/1.1 | brownish gray   | N 5.5/         | medium gray       |              |

#### Color 6: light grayish yellowish brown

**Hex:** #B49B8D

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 9.3YR 6.5/2.3 | pale yellowish brown | 9.3YR 6.5/2.3  | pale yellowish brown |              |

#### Color 7: light grayish olive

**Hex:** #8E856F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 8.2Y 5.5/2.2 | pale olive      | 8.2Y 5.5/2.2   | pale olive        |              |

#### Color 8: light olive gray

**Hex:** #8F877F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 7.3Y 5.6/1.2 | olive gray      | N 5.6/         | medium gray       |              |

#### Color 9: light greenish gray

**Hex:** #BABEC1

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 3.6G 7.6/0.8 | greenish gray   | N 7.6/         | light gray        |              |

#### Color 10: dark greenish gray

**Hex:** #545858

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 2.5G 3.6/0.8 | greenish gray   | N 5.0/         | medium gray       |              |

#### Color 11: light bluish gray

**Hex:** #B7B8C6

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 8.8B 7.4/0.9 | bluish gray     | N 7.4/         | light gray        |              |

#### Color 12: dark bluish gray

**Hex:** #50545F

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 0.1PB 3.5/1.0 | bluish gray     | N 3.5/         | dark grey         |              |

#### Color 13: light purplish gray

**Hex:** #C3B7C6

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 8.6P 7.5/1.1 | purplish gray   | N 5.0/         | medium gray       |              |

#### Color 14: dark purplish gray

**Hex:** #5C525E

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 8.3P 3.6/0.9 | purplish gray   | N 3.6/         | dark grey         |              |

#### Color 15: vivid purplish red

**Hex:** #DD2388

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 6.1RP 4.9/16.5 | vivid purplish pink | 6.0RP 4.9/16.7 | vivid purplish pink |              |

#### Color 16: strong purplish red

**Hex:** #B83773

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 6.8RP 4.4/12.0 | strong purplish pink | 6.8RP 4.4/12.2 | strong purplish pink |              |

#### Color 17: light grayish purplish red

**Hex:** #B2879B

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 5.5RP 6.0/4.1 | pale purplish red | 5.5RP 6.0/4.1  | pale purplish red |              |

#### Color 18: dark gray

**Hex:** #585458

| Illuminant | Rust Munsell | Rust descriptor | Python Munsell | Python descriptor | Dist polygon |
|------------|--------------|-----------------|----------------|-------------------|--------------|
| D65        | 2.1Y 3.6/0.1 | dark grey       | N 3.6/         | dark grey         |              |

### Python Errors

