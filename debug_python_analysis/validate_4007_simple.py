#!/usr/bin/env python3
"""Simple sequential validation of all 4007 reference colors"""
...
with open('tests/data/srgb-to-munsell.csv', 'r') as f:
    ...
    for row in reader:
        ...
        # Get Rust result - suppress stderr
        ...
        if total % 100 == 0:
            ...
            remaining = (4007 - total) / rate if rate > 0 else 0
            sys.stdout.write(f"\rProcessed {total}/4007 colors... ({100*total/4007:.1f}%, ~{remaining:.0f}s remaining)    ")
            sys.stdout.flush()
...
if family_matches >= 0.99 * total:
    print(f"✅ Family accuracy ≥ 99%: PASSED ({100*family_matches/total:.2f}%)")
else:
...
if chromatic_count > 0:
    if hue_within_01 >= 0.85 * chromatic_count:
        print(f"✅ Hues within 0.1 ≥ 85%: PASSED ({100*hue_within_01/chromatic_count:.1f}%)")
    else:
        print(f"❌ Hues within 0.1 ≥ 85%: FAILED ({100*hue_within_01/chromatic_count:.1f}%)")
