...
mod tests {
    ...
    fn debug_angle_calculation() {
        ...
        // Python says this should be 3.6YR (code 2), angle 171.6°
        // But hue_angle_to_hue(171.6°) gives BG, not YR
        // The issue is that Python's algorithm is getting a different spec from the angle!
        
        println!("\nPython expects: 3.6YR (code 2) from angle 171.6°");
        println!("But hue_angle_to_hue(171.6°) returns BG!");
        println!("This means the issue is NOT in hue_angle_to_hue but in the convergence algorithm");
    }
}